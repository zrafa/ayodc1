...
\begin{frame}
	...
	\textit{Architecture”/Instruc>on	Set	Architecture:}	
\begin{itemize}
	\item Programmer	visible	state	(Memory	and	Register)	
	\item Operaons	(Instrucons	and	how	they	work)	
	\item Execuon	Semancs	(interrupts)	
	\item Input/Output
	\item Data	Types/Sizes	
\end{itemize}
	\textit{Microarchitecture/Organizaon:}	
\begin{itemize}
	\item Tradeoffs	on	how	to	implement	ISA	for	some	metric	
(Speed,	Energy,	Cost)	
\item Examples:	Pipeline	depth,	number	of	pipelines,	cache	
size,	silicon	area,	peak	power,	execu>on	ordering,	bus	
widths,	ALU	widths	
\end{itemize}
\end{frame}
